...
const Services = ({ cardsPerView }: ServiceProps) => {
  ...
  const nextService = () => {
    setCurrentServiceIndex(
      (prevIndex) => (prevIndex + cardsPerView) % services.length
    );
  };
  ...
  const prevService = () => {
    setCurrentServiceIndex(
      (prevIndex) =>
        (prevIndex - cardsPerView + services.length) % services.length
    );
  };
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="overflow-hidden">
            <div
              ...
              style={{
                transform: `translateX(-${currentServiceIndex * (100 / cardsPerView)}%)`,
              }}
            >
              {services.map((service, index) => (
                <div
                  key={index}
                  className={`w-full ${cardsPerView === 3 ? "md:w-1/3" : cardsPerView === 2 ? "sm:w-1/2" : "w-full"} flex-shrink-0 px-2 sm:px-4 mb-8 sm:mb-0`}
                >
